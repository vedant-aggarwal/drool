  ...
  return <div className="mx-auto max-w-4xl space-y-6 p-4 sm:p-6 text-gray-200">
    ...
    <section className={card} aria-labelledby="codex-title">
      ...
      {connection?.accountType && <>
        ...
        <details><summary className="min-h-10 cursor-pointer py-2 text-sm">Persona and default instructions</summary><label className="block text-sm">Codex persona<textarea className={input + ' min-h-24'} value={persona} disabled={!!busy} onChange={e => { setPersona(e.target.value); localStorage.setItem('drool-codex-persona', e.target.value); setThread(undefined); setHistory([]) }} placeholder="You are my creative writing partner. Prefer concise answers and consistent characters…"/></label><p className="text-xs text-gray-400">Saved on this device. Changes start a new conversation.</p></details>
