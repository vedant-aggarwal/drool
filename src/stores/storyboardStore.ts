...
export interface StoryCharacter { id: string; name: string; appearance: string; personality: string; lora: string }
export interface StoryPanel { id: string; title: string; prompt: string; caption: string; approved: boolean; image?: GalleryItem }
...
interface StoryState {
  projects: StoryProject[]; activeId: string | null
...
export const useStoryboardStore = create<StoryState>()(persist((set) => ({
  ...
  updatePanel: (projectId, panelId, patch) => set(s => ({ projects: s.projects.map(p => p.id === projectId ? { ...p, updatedAt: Date.now(), panels: p.panels.map(panel => panel.id === panelId ? { ...panel, ...patch, id: panelId, approved: patch.prompt !== undefined && patch.prompt !== panel.prompt ? false : (patch.approved ?? panel.approved) } : panel) } : p) })),
}), { name: 'drool-storyboards-v1', storage: safeJSONStorage(), partialize: s => ({ activeId: s.activeId, projects: s.projects.map(p => ({ ...p, panels: p.panels.map(panel => ({ ...panel, image: panel.image ? { ...panel.image, dataUrl: undefined, remoteUrl: undefined } : undefined })) })) }) }))
